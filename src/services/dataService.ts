...
export const LS_KEYS = {
    ...
    // Metadata keys
    HOLDINGS_META: 'FIPO_HOLDINGS_METADATA',
    BOND_MASTER_META: 'FIPO_BOND_MASTER_METADATA',
    BENCHMARK_HOLDINGS_META: 'FIPO_BENCHMARK_HOLDINGS_METADATA',
};
...
// --- METADATA FUNCTIONS ---
export const saveFileMetadata = (key: string, metadata: FileMetadata): void => {
    try {
        localStorage.setItem(key, JSON.stringify(metadata));
    } catch (e) {
        console.error(`Error saving metadata for key ${key} to localStorage`, e);
    }
}

export const loadFileMetadata = (key: string): FileMetadata | null => {
    try {
        const storedData = localStorage.getItem(key);
        return storedData ? JSON.parse(storedData) : null;
    } catch (e) {
        console.error(`Error loading metadata for key ${key} from localStorage`, e);
        return null;
    }
}
